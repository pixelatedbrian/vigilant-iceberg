...
def main():
    ...
    file_path = ".indigo_model_weights.hdf5"
    ...
    if score_test is not None:
        ...
        test_pics = data_pipeline(df_test, standardized_params)

        test_inc_angle = pd.to_numeric(df_test.loc[:, "inc_angle"], errors="coerce")
        test_inc_angle[np.isnan(inc_angle)] = inc_angle.mean()
        # inc_angle = np.array(inc_angle, dtype=np.float32)

        # TODO: enable this?
        # has the (mean, std) from standardizing inc_angle earlier
        test_inc_angle, inc_std_params = standardize(inc_angle, inc_std_params)
