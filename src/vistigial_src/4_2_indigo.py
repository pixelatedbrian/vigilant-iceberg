...
def main():
    ...
    file_path = ".indigo_model_v4_2_weights.hdf5"
    ...
    if score_test is not None:
        ...
        print("Read in test data to be scored.")

        df_test = pd.read_json('../data/test.json')

        print("Push through data pipeline...")
        test_pics, _ = data_pipeline(df_test, standardized_params, special_c3=False)

        print("pipeline inc_angle")
        ...
        test_inc_angle[np.isnan(test_inc_angle)] = inc_std_params[0]
        # inc_angle = np.array(inc_angle, dtype=np.float32)
